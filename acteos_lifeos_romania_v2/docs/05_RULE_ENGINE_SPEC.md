# 05 — Event Resolver & Deterministic Rule Engine

## 1. Scop

Motorul transformă un `EventOccurrence` și un set de fapte într-un `Journey`. El nu caută pe web în timpul rezolvării și nu apelează LLM-uri. Primește un ruleset publicat și produce un rezultat reproductibil.

## 2. Contractul de intrare

```json
{
  "event_type_id": "ro.life.moved_home",
...
- `LifeEventType`: definiție stabilă a evenimentului;
- `EventOccurrence`: instanța utilizatorului;
...
3. selectează ruleset-ul activ pentru eveniment, dată și jurisdicție;
...
def resolve(request, ruleset):
    ...
    candidates = ruleset.select(canonical.event_type_id,
