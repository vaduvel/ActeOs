...
`Home → event candidate → triage → resolve → Journey dashboard → step → requirements/readiness → official channel → outcome/feedback`.
...
For P0, scaffold the monorepo and CI only. Do not implement real administrative routes. Use the synthetic fixtures solely to prove build/test plumbing. Then proceed with the phase prompts.
