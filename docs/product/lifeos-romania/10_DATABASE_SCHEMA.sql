-- LifeOS Romania canonical schema (superbook)
-- Productie. Reutilizeaza tabelele existente unde e cazul; adauga stratul de eveniment.
...
CREATE TABLE IF NOT EXISTS life_event_types (
  ...
  label TEXT NOT NULL,
  frequency_tier TEXT NOT NULL CHECK (frequency_tier IN ('high','medium','low')),
  r1_scope BOOLEAN NOT NULL DEFAULT false,
  definition JSONB NOT NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Legatura eveniment -> obligatie, cu dependente (graf)
CREATE TABLE IF NOT EXISTS event_procedure_refs (
  id UUID PRIMARY KEY,
  event_type_id TEXT NOT NULL REFERENCES life_event_types(id) ON DELETE CASCADE,
  obligation_id TEXT NOT NULL,
  obligation TEXT NOT NULL CHECK (obligation IN ('mandatory','conditional','optional','later')),
  applies_when JSONB NOT NULL DEFAULT '{}'::jsonb,
  depends_on TEXT[] NOT NULL DEFAULT '{}',
  phase_order INTEGER NOT NULL DEFAULT 0,
  rationale_claim_ids TEXT[] NOT NULL DEFAULT '{}',
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS sources (
  ...
  freshness_class TEXT NOT NULL DEFAULT 'B' CHECK (freshness_class IN ('A','B','C')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS source_claims (
  ...
  effective_from DATE,
  effective_to DATE,
  confidence TEXT NOT NULL CHECK (confidence IN ('verified','verified_with_local_gap','needs_confirmation','conflicting','expired','withdrawn')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- Plan de eveniment (orchestrator) + traseele per nod
...
CREATE TABLE IF NOT EXISTS route_runs (
  id UUID PRIMARY KEY,
  session_id UUID REFERENCES event_sessions(id) ON DELETE CASCADE,
  plan_id UUID REFERENCES event_plans(id) ON DELETE CASCADE,
  obligation_id TEXT,
  bundle_id TEXT REFERENCES rule_bundles(id),
  facts_hash TEXT NOT NULL,
