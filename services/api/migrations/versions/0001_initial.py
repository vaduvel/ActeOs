"""Canonical schema: content / app / ops / audit.

Revision ID: 0001
Revises:
Create Date: 2026-06-25

This migration is the *executable* source of truth for the database. It mirrors
`docs/product/waze-birocratie/09_DATABASE_SCHEMA.sql` 1:1 (PostgreSQL 16+). The
SQL document remains the human-readable canonical reference; the two MUST stay
in lockstep. The schema is Postgres-only by design (ADR-003): transactions,
JSONB, enums, append-only audit triggers and partial unique indexes are part of
the correctness model, not incidental.
"""
...
# NOTE: kept verbatim from the canonical DDL minus the outer BEGIN/COMMIT, which
# Alembic provides via its own per-migration transaction. Executed through the
# driver (exec_driver_sql) so Postgres dollar-quoted function bodies and the
# multi-statement script run untouched by SQLAlchemy bind-parameter parsing.
CANONICAL_DDL = r"""
...
def upgrade() -> None:
    op.get_bind().exec_driver_sql(CANONICAL_DDL)


def downgrade() -> None:
    op.get_bind().exec_driver_sql(DROP_DDL)
