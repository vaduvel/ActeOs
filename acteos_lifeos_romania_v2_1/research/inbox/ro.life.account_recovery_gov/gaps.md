# Gaps — ro.life.account_recovery_gov
...
|---|---|---|---|
| `gap.other_platforms` | `critical` | Nu există flux universal pentru toate conturile publice și UAT-urile. | Research per platformă; altfel needs_confirmation. |
| `gap.no_contact_access` | `critical` | Când utilizatorul nu mai are e-mailul/telefonul asociat, rezolvarea poate necesita suport și verificare suplimentară nepublicată. | Canal oficial de suport și confirmare instituțională. |
| `gap.security_incident` | `critical` | Fluxurile publice cercetate nu acoperă uniform compromiterea contului și revocarea sesiunilor. | Research separat pe incident de securitate per platformă. |
