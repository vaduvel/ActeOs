...
### `intent_categories` / `intent_types` / `intent_aliases`

Catalogul public de obiective, aliasuri și ranking metadata. Nu conține obligații administrative.

### `intent_event_links`

Leagă intenturile directe și bundle-urile de contexte de viață.
...
În v2.1, `intent_type_id` este cheia publică obligatorie. `event_type_id` și `event_context_ids` sunt context opțional/legacy.

Demers început de utilizator, identificat prin `intent_type_id`, subiect, reference date, jurisdiction și lifecycle.
...
### `intent_query_feedback`

Telemetrie minimă pentru result bucket și selecție; nu păstrează query brut.
...
Analytics folosește pseudonymous installation/account id, `intent_type_id`, `category_id`, score band, state codes și durations. Nu include:
