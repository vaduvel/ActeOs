...
O procedură singulară, precum înscrierea la grădiniță, este utilă dar sezonieră. Produsul devine recurent când omul poate găsi rapid orice obiectiv administrativ frecvent, iar sistemul leagă obiectivele conexe într-un context continuu pentru persoană sau familie: mutare, vehicul, documente, taxe, naștere, muncă, proprietate, firmă și expirări.

Wedge-ul nu este „catalog de acte”. Wedge-ul este **discovery-ul rapid al demersului potrivit plus orchestrarea personalizată a traseelor cu frecvență și fricțiune ridicate**, pornind din Timișoara și extinzând baza națională.
...
- „Când vreau să rezolv X, găsește demersul corect și spune-mi tot traseul, nu doar primul formular.”
...
- Home: „Ce vrei să rezolvi?” + search, quick actions, continuare cazuri și categorii;
- rezultate intent + clarificare candidat;
- triage facts;
- Journey Dashboard;
...
- catalog, normalizare, intent search și disambiguation;
- case/facts;
- resolver;
...
1. `intent_query_started` sau `category_opened`
2. `intent_candidate_selected`
3. `triage_started`
4. `triage_completed`
5. `journey_generated`
6. `first_step_viewed`
...
PII și query-ul brut nu intră în analytics. Se păstrează doar intent_type, result bucket, stare, timpi și coduri agregabile aprobate de privacy review.
...
- search-ul intent-first crește activarea, iar bundle-urile event-aware generează valoare mai mare decât procedurile izolate;
