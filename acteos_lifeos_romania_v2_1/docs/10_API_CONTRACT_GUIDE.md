...
- `GET /categories`
- `GET /intents`
- `POST /intents/resolve-query`
- `GET /intents/{intent_type_id}`
- `GET /life-events` (context/legacy)
- `POST /life-events/classify` (deprecated compatibility)
...
## 5A. Discovery semantics

Search-ul nu creează caz și nu returnează mai mult de trei candidați. Fiecare răspuns include catalog/resolver/normalization version. `POST /cases` cere intent confirmat. Vezi `10A_DISCOVERY_API_CONTRACT.md`.
