...
## D1. Intrarea este intenția administrativă

Interfața principală întreabă „Ce vrei să rezolvi?”. Utilizatorul poate căuta direct sau poate explora categoriile. Instituția, formularul și denumirea oficială nu sunt condiții pentru a începe. Query-ul se rezolvă într-un `intent_type_id` canonic, iar utilizatorul îl confirmă înainte de crearea cazului.

## D1A. Evenimentul de viață este context, nu obligație de navigație

Event Atlas rămâne stratul care compune bundle-uri și recomandă intenturi conexe: o mutare poate activa schimbarea actului, a talonului, taxelor și utilităților. Utilizatorul poate însă porni direct de la oricare dintre aceste obiective. Discovery este intent-first; orchestration este event-aware.
...
Eligibilitatea, includerea unui act, calculul termenului, blocarea unui pas și starea de pregătire sunt produse de reguli tipate. Un LLM poate propune numai candidați dintre intenturile publicate, poate extrage câmpuri din text și poate explica rezultatul, dar nu poate crea intenturi, porni automat un caz sau modifica rezultatul motorului.
...
## D19A. Search-ul este catalog-gated și neutru

Ranking-ul poate folosi potrivirea lexicală, disponibilitatea, recența locală și utilitatea agregată. Nu poate folosi sponsorizarea. AI fallback-ul poate returna numai ID-uri canonice, iar query-ul brut sensibil nu intră în analytics.
