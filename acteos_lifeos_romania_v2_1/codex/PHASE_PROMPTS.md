...
Validate OpenAPI/JSON Schemas including intent/discovery contracts, introduce lint/diff tooling, generate TypeScript client and Python transport models where appropriate. Acceptance: generated code is reproducible and no duplicate handwritten DTOs exist.

## P2 — Domain model, Intent Atlas and predicate DSL

Implement intent/category value objects, alias normalization contracts, intent-event links, enums, canonical serialization, predicate models/parser and errors. No DB or FastAPI imports. Acceptance: schema roundtrip and invalid AST cases covered.

## P3 — Deterministic intent and rule engines

Implement Romanian normalizer, alias index, ranking/disambiguation/no-result, then evaluator, temporal/jurisdiction/freshness/conflict/graph/materialization/trace. Use only testdata. Acceptance: property tests and canonical hash determinism pass.
...
## P5 — Discovery + API vertical slice

Implement discovery home, categories, intent list/resolve/detail, then case create/get, facts update, resolve and journey get. Add auth modes, idempotency, errors and contract tests. Acceptance: synthetic event resolves end-to-end through HTTP.
...
## P7 — Discovery and Journey UX

Implement Home search, categories, candidates, disambiguation, no-result, recent/favorites, triage, dashboard, step, requirements, trust states, official channel and feedback shell. Acceptance: canonical synthetic flow passes Maestro and accessibility smoke.
